# 1 "C:\\Users\\louis16s\\AppData\\Local\\Temp\\tmpfvb71aef"
#include <Arduino.h>
# 1 "D:/users/desktop/fish1/src/MAIN_ALL.ino"
...
void setup();
void loop();
#line 111 "D:/users/desktop/fish1/src/MAIN_ALL.ino"
static void Gate_Log(const char* fmt, ...)
{
...
{
  for (uint8_t i = 0; i < CtrlCfg.cycle_count && i < 2; i++) {
...
static void Update_Alarm_Status()
{
  ...
  if (alarm_gate_timeout || alarm_sensor_timeout) {
    Alarm_Active = true;
    Alarm_Severity = 2;
    if (alarm_gate_timeout) {
