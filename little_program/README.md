...
1. 配置检查
- [ ] `utils/config.js` 的 `BASE_URL` 为 HTTPS 正式域名
- [ ] 微信后台 `request 合法域名` 与 `BASE_URL` 一致
- [ ] 服务端 `healthz` 全绿（`ok/mqtt/db`）

2. 功能回归
- [ ] 登录/退出登录流程正常
- [ ] 主控页可切换设备、可执行开关闸命令
- [ ] 配置页可读取并保存规则（cache miss 时可回退 RPC）
- [ ] 回放页可查询区间并导出 JSON/CSV
- [ ] 管理员页用户管理与保留天数设置可用

3. 风险点
- [ ] 网络较差时，命令超时提示文案可理解（504）
- [ ] 非 admin 访问管理接口时提示正确（403）
- [ ] 空数据设备（新设备）页面不崩溃
